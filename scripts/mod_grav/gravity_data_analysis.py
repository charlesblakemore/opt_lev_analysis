...
data_dirs = ['/data/new_trap/20230330/Bead0/Gravity/2/']
new_trap = True
...
mle_vs_time_chunk_size = 500
zoom_limits = ()
# zoom_limits = (6.0, 6.5)
plot_freqs = [18.0, 24.0, 33.0, 36.0]
plot_freqs = [9.0, 12.0, 15.0, 18.0, 21.0, 24.0, 27.0, \
              30.0, 33.0, 36.0, 39.0]
plot_alpha = 1.0
...
derp_save = True
derp_file = '/home/cblakemore/tmp/20230330_mle_vs_time.p'
# derp_file = '/home/cblakemore/tmp/20230330_noise_mle_vs_time.p'
derp_key = 'naive'
# derp_key = 'rand{:d}'.format(arg3)
# derp_nchunk = int(round(Nfiles/file_chunking))
derp_nchunk = int(Nfiles/500)
########### BAD CODE #############
...
export_limit = True
export_path = '/data/new_trap_processed/limits/20230330_limit_binning-10000.p'
...
for ddir in data_dirs:
    ...
    aux_path_base = ddir.replace('/data/new_trap/', \
                                 '/data/new_trap_processed/processed_files/')
    ...
    paths = gu.build_paths(ddir, opt_ext, \
                           pardirs_in_name=pardirs_in_name, \
                           new_trap=new_trap)
    ...
    if reprocess:
        ...
        datafiles, lengths = bu.find_all_fnames(\
                                    ddir, ext=config.extensions['data'], \
        ...
        if plot_harms or plot_tf: 
            ncore_now = 1
        else:
            ncore_now = ncore
        agg_dat = gu.AggregateData(\
                        datafiles, p0_bead=p0_bead, harms=harms, \
                        plot_harm_extraction=plot_harms, \
                        plot_tf=plot_tf, new_trap=new_trap, \
                        step_cal_drive_freq=71.0, ncore=ncore_now, \
                        noisebins=10, aux_data=aux_data, \
        ...
        if plot_templates:
            ...
            ncore_now = ncore
        agg_dat.find_alpha_likelihoods_every_harm(
                    ncore=ncore_now, plot_templates=plot_templates, \
        ...
        if redo_alpha_fit:   
            # agg_dat.find_alpha_xyz_from_templates(\
            #             plot=plot_alpha_xyz, plot_basis=plot_basis, \
    ...
    if plot_chunked_mle_vs_time:
        agg_dat.plot_chunked_mle_vs_time(\
                    show=show_plots, save=save_plots, \
                    plot_freqs=plot_freqs, \
                    basepath=plot_dir, plot_alpha=plot_alpha, \
    ...
    if plot_mle_histograms:
        agg_dat.plot_mle_histograms(\
                    show=show_plots, save=save_plots, \
                    bins=20, basepath=plot_dir)

    if plot_likelihood_ratio_histograms:
        for lambda_to_plot in lambdas_to_plot:
            agg_dat.plot_likelihood_ratio_histograms(\
                        show=show_plots, save=save_plots, \
                        basepath=plot_dir, \
    ...
    if plot_harmonic_likelihoods:
        for lambda_to_plot in lambdas_to_plot:
            agg_dat.plot_sum_likelihood_by_harm(\
                        show=show_plots, save=save_plots, \
                        basepath=plot_dir, \
    ...
    if plot_limit:
        agg_dat.get_limit_from_likelihood_sum(\
                    confidence_level=confidence_level, \
                    no_discovery=no_discovery, ss=ss, \
                    xlim=limit_xlim, ylim=limit_ylim,
                    show=show_plots, save=save_plots, \
                    basepath=plot_dir, export_limit=export_limit, \
