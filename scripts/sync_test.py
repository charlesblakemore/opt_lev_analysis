...
dirname = '/data/20180604/sync_test_27/'
...
for filname in files[:1000]:
    ...
    posdat_range = np.max(df.pos_data[2]) - np.min(df.pos_data[2])
    cantdat_range = np.max(df.cant_data[2]) - np.min(df.cant_data[2])
    ...
    ax.plot(df.pos_time, (df.pos_data[2] - np.mean(df.pos_data[2])) * fac + 5, '--', \
            lw=3, label='Phase')
    ax.plot(df.daqmx_time, df.cant_data[2], label='Attractor Monitor')
