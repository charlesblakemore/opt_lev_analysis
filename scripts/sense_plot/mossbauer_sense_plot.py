...
annotate = True
annotate_bbox = {'boxstyle': 'round,pad=0.1', 'fc': 'none', 'ec': 'none', 'alpha': 0.75}
...
projection_paths = [ \
                    './projections/mossbauer_fe57_down_quark_modulus_sensitivity.txt', 
                    './projections/mossbauer_ta181_down_quark_modulus_sensitivity.txt', 
                   ]
projection_facs = [ \
                   1.0, \
                   1.0, \
...
# axis_scale = (1e6, r'$\\mu$m')
axis_scale = (1.0, 'm')

xlim = (9e-10, 4e-5) # limit in meter
ylim = (9e1, 1.1e23)
...
savefig = True
fig_path = '/home/cblakemore/plots_mossbauer/alpha_lambda_exclusions_submicron_projections.svg'
...
if plot_projection:
    ...
    for i, fac in enumerate(projection_facs):
        projection_data = np.loadtxt(projection_paths[i], delimiter=',')
        label = projection_labels[i]
        ls = projection_linestyles[i]
        color = projection_colors[i]
        alphalim = projection_alphalim[i]

        lambdas = projection_data[:,0]
        limit = fac * projection_data[:,1]
...
ax.text(9e-6, 1.0e18, 'EXCLUDED BY\nEXPERIMENTS', \
...
projection_data = pickle.load( \
        open('/data/new_trap_processed/limits/20200320_noise-limit_discovery_closer_scaled.p', 'rb') )
alphalim = (-np.inf, 1e13)
lambdas = projection_data['pos_limit'][0] * axis_scale[0]
limit = np.max( np.abs(np.stack((projection_data['pos_limit'][1], \
                                 projection_data['neg_limit'][1]))), axis=0)
limit *= np.sqrt(10000.0 / (30.0 * 24.0 * 3600.0))

if len(alphalim):
    inds = (limit > alphalim[0]) * (limit < alphalim[1])
else:
    inds = limit > -np.inf
ax.loglog(lambdas[inds], limit[inds], color=microspheres_color, lw=3, \
            ls=(0, (3, 1.5, 1, 1.5, 1, 1.5)), zorder=5)

if annotate:
    ax.annotate('', zorder=4,\
                xy=(1.6e-6, 2.5e5), xytext=(2.3e-6, 2e7), \
                arrowprops={'width': 1, 'headwidth': 3, 'headlength': 3, \
                            'shrink': 0.05, 'color': microspheres_color, \
                            'alpha': text_alpha*0.7})
    ax.annotate('Microsphere\nsensitivity', \
                xy=(1.6e-6, 2.5e5), xytext=(2.3e-6, 2e7), \
                ha='center', va='bottom', fontsize=10, fontweight='bold',\
                zorder=4, alpha=text_alpha, color=microspheres_color, \
                bbox=annotate_bbox)
...
ax.loglog(cmeas[:,0]*axis_scale[0],cmeas[:,1],'C2',alpha=line_alpha,linewidth=data_linewidth, zorder=4)
if annotate:
    ax.annotate('', zorder=4,\
                xy=(8.6e-6, 11000), xytext=(1e-6, 1e3), \
                arrowprops={'width': 1, 'headwidth': 3, 'headlength': 3, \
                            'shrink': 0.05, 'color': 'C2', \
                            'alpha': text_alpha*0.7})
    ax.annotate('Lee et al.\nPRL 124 101101 (2020)', \
                xy=(8.6e-6, 11000), xytext=(2.9e-7, 1e3), \
                ha='center', va='center', fontsize=10, fontweight='bold',\
                zorder=4, alpha=text_alpha, color='C2', \
                bbox=annotate_bbox)


cmeas = np.loadtxt('prev_meas/chen_prl_116_221102_2016.txt',delimiter=",",skiprows=0)
ax.loglog(cmeas[:,0]*axis_scale[0],cmeas[:,1],'C0',alpha=line_alpha,linewidth=data_linewidth, zorder=4)
if annotate:
    ax.annotate('', zorder=4,\
                xy=(2.1e-7, 1e9), xytext=(7.9e-8, 2e8), \
                arrowprops={'width': 1, 'headwidth': 3, 'headlength': 3, \
                            'shrink': 0.05, 'color': 'C0', \
                            'alpha': text_alpha*0.7})
    ax.annotate('Chen et al.\nPRL 116 221102 (2016)', \
                xy=(2.1e-7, 1e9), xytext=(2.0e-8, 2e8), \
                ha='center', va='center', fontsize=10, fontweight='bold',\
                zorder=4, alpha=text_alpha, color='C0', \
                bbox=annotate_bbox)
...
inds = cmeas[:,0] < 2e-7
ax.loglog(cmeas[:,0][inds]*axis_scale[0],cmeas[:,1][inds],'C1',alpha=line_alpha,\
          linewidth=data_linewidth, zorder=4)
if annotate:
    ax.annotate('', zorder=4,\
                xy=(7.0e-8, 8e16), xytext=(6.0e-8, 1.0e20), \
                arrowprops={'width': 1, 'headwidth': 3, 'headlength': 3, \
                            'shrink': 0.05, 'color': 'C1', \
                            'alpha': text_alpha*0.7})
    ax.annotate('Leeb et al.\nPRL 68 1472 (1992)', \
                xy=(7.0e-8, 8e16), xytext=(6.0e-8, 1.0e20), \
                ha='center', va='bottom', fontsize=10, fontweight='bold',\
                zorder=4, alpha=text_alpha, color='C1', \
                bbox=annotate_bbox)


cmeas = np.loadtxt('prev_meas/nesvizhevsky_prd_77_034020_2008b.txt',delimiter=",",skiprows=0)
ax.loglog(cmeas[:,0]*axis_scale[0],cmeas[:,1],'C1',alpha=line_alpha,linewidth=data_linewidth, zorder=4)
if annotate:
    ax.annotate('', zorder=4,\
...
# ax.loglog(cmeas[:,0]*axis_scale[0],cmeas[:,1],'k',linewidth=linewidth, zorder=4)
# if annotate:
#     if ref:
