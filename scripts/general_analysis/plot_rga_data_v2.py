...
directory = ['/daq2/20190626/bead1/spinning/pramp/N2_2/rga/']
...
def plot_rga_data(lines, skip_ind = 0):
	...
	part_press = scan_data['scans'][skip_ind:]
	tot_pressure = scan_data['pressures'][skip_ind:]
...
def plot_rga_data_(data_file, skip_ind = 0):
	data = ru.get_rga_data(data_file, all_scans = False,scan_ind = scan_ind, plot=False, plot_many=False)
...
for i in range(end_ind):
	...
	leak_part_press, leak_pressure, leak_mass_vec = plot_rga_data(lines, skip_ind = i-end_ind)
	tot += leak_part_press
	
	
	
mean_part_press = tot/end_ind
