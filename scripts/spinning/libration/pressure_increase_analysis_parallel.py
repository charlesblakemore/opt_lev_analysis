...
#path_list = ['/data/old_trap/20200330/gbead3/spinning/pramp_2/25kHz_4Vpp_xy_1/',\
...
def proc_data(f):
    try:
        ...
        if plot_dat:
        	...
        	plt.show()
        phase = np.angle(np.sum(fft))
        dphase = np.angle(np.sum(fft2))
        pressure = obj.attribs["pressures"]
        time = obj.attribs['time']
    except:
        print "bad file"
    
    return phase, dphase, pressure, time
for i, path in enumerate(path_list):
    ...
    phase, dphase, pressure, time = zip(*Parallel(n_jobs=n_jobs)(delayed(proc_data)(f) for f in files))    
...
if save:
    np.save(save_name + '_efield.npy', efield)
