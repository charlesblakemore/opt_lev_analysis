...
plot_name = 'libration_ringdown_examples_log.svg'
...
for fileind, filename in enumerate(filenames):
    ...
    for phi_dg_ind, phi_dg in enumerate(phi_dg_to_plot):
        ...
        ax.plot(special_plot_x, fit_func(special_plot_x+start_time, *fit), \
                        color=color, lw=4, ls='--', zorder=5+int(n_dg-phi_dg_ind), \
                        label=label)
        ...
        if special_nplotgap:
            ...
            if phi_dg == example_dg_to_plot:
                ...
                ax_inset.plot(special_plot_x, special_plot_y_amp, color=color, alpha=1.0, \
                                zorder=4, lw=2, label=special_label)
                ax_inset.set_xlim(-0.1*tau, efoldings_to_plot*tau)
    ...
    legend = ax.legend(loc='lower right', ncol=2, fontsize=12, framealpha=1, \
                       title='$k_d$ [arb],  $\\hat{\\tau}$ [s]')
    legend.set_zorder(99)
    ax_inset.legend(loc='lower right', fontsize=12, framealpha=1)
