...
dlist = [0.625, 1, 1.75, 2.5,] ##[1.125, 2.25, 3.5, 4.625]
aclist = [200, 500, 500, 2000,]
vdc_list = [-6.2,-6.1,-6,-5.9,-5.8,-5.7,-5.6,-5.5,-5.4,-4.5,-4.1,-3.9,-3.8,-3.4]
path = "/data/20150202/Bead1/cantidrive"
fdrive = 35 ## Hz

## make the list of possible dc values
all_list = glob.glob( os.path.join(path,"*.h5"))
for f in all_list:
    curr_str = re.findall( "m?\d+\.?\d*dc", f)[0]
    if( curr_str[0] == 'm' ):
        vdc_list.append(-1*float( curr_str[1:-2] ) )
    else:
        vdc_list.append( float( curr_str[0:-2] ) )
vdc_list = sorted(vdc_list)
print vdc_list
#raw_input()
plt.figure()
...
for col,d,acv in zip(col_list,dlist,aclist):
    ...
    for vdc in vdc_list:
        ...
        vstr = vstr.replace("-","m")
        vstr = vstr.replace(".","_")
        if( vstr[-2:] == "_0"): vstr = vstr[:-2]
        if( vstr == "m4" and dstr == "1_75" ):
            cname = "urmbar_xyzcool2_%s_%sdc2_%dmV_35Hz.h5"%(dstr, vstr, acv)
        else:
            cname = "urmbar_xyzcool2_%s_%sdc_%dmV_35Hz.h5"%(dstr, vstr, acv)
        flist.append( cname )
        #cname = "urmbar_xyzcool2_%s_%sdc2_%dmV_35Hz.h5"%(dstr, vstr, acv)
        #flist.append( cname )
