...
data_dir_list = ["/data/20151208/bead1/amp_sweep2", 
                 "/data/20151208/bead1/amp_sweep",
                 "/data/20151208/bead1/amp_sweep3",]
...
def get_curr_freq( s ):
    return float(re.findall( "elec3_\d+mV", s )[0][6:-2])
...
for ddid,data_dir in enumerate(data_dir_list):
    ...
    for i,f in enumerate(flist):
        ...
        if( ddid == 0 ):
            if( fidx==0 ):
                plt.semilogy( freqs, y, label = str(fex*200)+" V", color=scalarMap.to_rgba(color_idx) )
            else:
