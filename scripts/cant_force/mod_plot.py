...
data_path = "/data/20150603/Bead2/mod_plots"
...
def get_avg_dat( cfile, col, dist ):
    ...
    plt.semilogy(freqs,np.sqrt(fr*(freqs[1]-freqs[0])*corr_fac), color=col, label=glab, linewidth=1)
    plt.xlim([35, 47])
...
for i,p in enumerate(plist):
    ...
    if(i == 1 ):
        fname = "urmbar_xyzcool_nomod_400mV_41Hz_200mVDC.h5"
        glab = "No modulation"
    else:
        fname = "urmbar_xyzcool_mod_400mV_41Hz_200mVDC.h5"
        glab = "With modulation"
...
plt.gcf().set_size_inches(6*0.8,4.5*0.8)
plt.subplots_adjust(bottom=0.125,left=0.15)
plt.savefig("mod_plot.pdf")
