...
class AggregateData:
    ...
    def save(self, savepath, verbose=True):
        ...
        if len(parts) > 2:
            ...
            if verbose:
                print 'Saving AggregateDate object... ',
            if parts[1] != 'agg':
                if verbose:
                    print
        ...
        for file_data_obj in self.file_data_objs:
            ...
            if len(ax1vec):
                ...
                if np.abs(ax1vec[close_ind] - ax1pos) < ax_disc:
                    ...
                    if old_ax1key not in agg_dict[bias][new_ax0key].keys():
                        if dim3:
                    ...
                    if old_ax1key != new_ax1key:
                        ...
                        for ax0key in ax0vec:
                            ax1keys = agg_dict[bias][ax0key].keys()
            ...
            if dim3:
                ...
                if len(ax2vec):
                    ...
                    if np.abs(ax2vec[close_ind] - ax2pos) < ax_disc:
                        ...
                        if old_ax2key not in agg_dict[bias][new_ax0key][new_ax1key].keys():
                        ...
                        if old_ax2key != new_ax2key:
                            ...
                            for ax0key in ax0vec:
                                ...
                                for ax1key in ax1keys:
                                    ax2keys = agg_dict[bias][ax0key][ax1key].keys()
    ...
    def handle_sparse_binning(self, dim3=False, verbose=False):
        ...
        for bias in self.biasvec:
            ax0keys = self.agg_dict[bias].keys()
            ax0keys.sort()
            ...
            for ax0 in ax0keys:
                ax1keys = self.agg_dict[bias][ax0].keys()
                ax1keys.sort()
                ...
                if dim3:
                    for ax1 in ax1keys:
                        ax2keys = self.agg_dict[bias][ax0][ax1].keys()
                        ax2keys.sort()
    ...
    def average_resp_by_coordinate(self):
        ...
        i = 0
        totlen = len(self.agg_dict.keys()) * len(self.ax0vec) * len(self.ax1vec)
        for bias, ax0, ax1 in itertools.product(self.agg_dict.keys(), self.ax0vec, self.ax1vec):
            i += 1
            newline=False
            if i == totlen:
                newline=True
