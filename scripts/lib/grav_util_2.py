...
class FileData:
    ...
    def rebuild_drive(self, mean_pos=40.0):
        ...
        temp_inds = np.array(list(range(int(self.nsamp*0.5) + 1)))
    ...
    def fit_alpha_xyz(self, tempsxyz, diag = False, \
        ...
        ndat = norm(dat)
        ns_temps = np.array(list(map(norm, tempsxyz)))
...
class AggregateData:
        ...
        for file_data_obj in self.file_data_objs:
            ...
            if len(ax2vec):
                ...
                if np.abs(ax2vec[close_ind] - ax2pos) < ax_disc:
                    ...
                    if old_ax2key not in list(agg_dict[bias][new_ax1key].keys()):
                    ...
                    if old_ax2key != new_ax2key:
                        ...
                        for ax1key in ax1vec:
                            ax2keys = list(agg_dict[bias][ax1key].keys())
    ...
    def find_mean_alpha_vs_position(self, ignoreXYZ=(0,0,0)):
        ...
        i = 0
        totlen = len(list(self.agg_dict.keys())) * len(self.ax1vec) * len(self.ax2vec)
        for bias, ax1, ax2 in itertools.product(list(self.agg_dict.keys()), self.ax1vec, self.ax2vec):
            i += 1
